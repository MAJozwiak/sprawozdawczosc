...
def combine_one_dimensional_data(lab_codes_labels_and_value: List[Tuple[str, str]]
    ...
    return dict_label_value_data_point, False  # Zwraca słownik oraz informację, czy arkusz jest dwuwymiarowy —
    # w tym przypadku False (arkusz jednowymiarowy)


def combine_two_dimensional_data(lab_codes_labels_and_value: List[Tuple[str, str]],
                                 lab_pl_labels_and_value: List[Tuple[str, str]],
                                 axis_x_labels: list[str], axis_y_labels: list[str]) -> Tuple[
    Dict[str, TwoDimensionalAxis], bool]:
    ...
    return dict_label_value_data_point, True  # Zwraca słownik oraz informację, czy arkusz jest dwuwymiarowy —
    # w tym przypadku True (arkusz dwuwymiarowy)


def combine_data_from_files(rend_labels: List[str],
                            lab_codes_labels_and_value: List[Tuple[str, str]],
                            lab_pl_labels_and_value: List[Tuple[str, str]]) -> Tuple[
    Union[Dict[str, TwoDimensionalAxis], Dict[str, OneDimensionalAxis]], bool]:
    """Funkcja została stworzona w celu połączenia danych z trzech plików XML: REND, LAB_CODES oraz LAB_PL.
       Każdy z plików dostarcza inne informacje:
        ...
        Dane będą później przetwarzany w celu dodania typów danych, nazw metryk, nazwy arkusza oraz nazwy formularza w
        celu uzyskania pełnej struktury danych."""

    if 'x' in rend_labels and 'y' in rend_labels:  # Sprawdzenie, czy w danych z pliku REND występują obie osie: "x" i
        # "y" (oznacza formularz dwuwymiarowy)
        idx_x = rend_labels.index('x')
        idx_y = rend_labels.index('y')

        # W pliku REND osie "x" i "y" mogą występować w różnej kolejności (np. [tax_c1, tax_c2, x, tax_c3, y] lub
        # [tax_c1, tax_c2, y, tax_c3, x]).
        ...
        # Mimo że warunek wykrył formularz jako dwuwymiarowy, może wystąpić sytuacja, w której do osi x nie jest
        # przypisany żaden label.
        # W takim przypadku traktujemy formularz jako jednowymiarowy.
        # Przykład takiej sytuacji: [tax_c1, tax_c2, y, x], gdzie wszystkie etykiety przypisane są do osi y, a oś x
        # zawiera np. tylko wymiar techniczny (jednak my wyciągamy tylko labele, wiec np. wymiar nie będzie zawarty).
        if len(axis_y_labels) == 0 or len(axis_x_labels) == 0:
            ...
            return combine_two_dimensional_data(lab_codes_labels_and_value, lab_pl_labels_and_value, axis_x_labels,
                                                axis_y_labels)
...
def match_labels_with_data_types(rend_labels_and_qnames: List[Tuple[str, str]], data_types: List[Tuple[str, str]]) -> \
        List[List[str]]:
    """Funkcja została stworzona po to, aby dopasować do każdego z podanych labeli odpowiednią metrykę (nazwę) oraz
    jej typ danych."""
...
def match_datatypes_and_qnames(labels_and_data_types: List[List[str]],
                               combine_data: Union[Dict[str, TwoDimensionalAxis], Dict[str, OneDimensionalAxis]]) -> \
        Union[Dict[str, TwoDimensionalAxis], Dict[str, OneDimensionalAxis]]:
    ...
    for key, type_val, name in labels_and_data_types:
